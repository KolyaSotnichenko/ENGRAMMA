...
const make_decay_cfg = (): decay_cfg => ({
    threads: parse_int(process.env.ENGRAMMA_DECAY_THREADS, 3),
    cold_threshold: parse_f(process.env.ENGRAMMA_DECAY_COLD_THRESHOLD, 0.25),
    reinforce_on_query: parse_bool(
        process.env.ENGRAMMA_DECAY_REINFORCE_ON_QUERY,
        true,
    ),
    regeneration_enabled: parse_bool(
        process.env.ENGRAMMA_REGENERATION_ENABLED,
        true,
    ),
